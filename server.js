...
export function createApp({dataDir=process.env.DATA_DIR||join(root,'data'),production=process.env.NODE_ENV==='production',origin=process.env.APP_ORIGIN||''}={}){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try{
   ...
   let body={};if(req.method!=='GET'){let raw='';for await(const chunk of req){raw+=chunk;if(Buffer.byteLength(raw)>100000)throw new InputError('Request too large.',413);}try{body=JSON.parse(raw||'{}');}catch{throw new InputError('Invalid JSON.');}if(!body||typeof body!=='object'||Array.isArray(body))throw new InputError('JSON object required.');}
   ...
   if(url.pathname==='/api/runs'&&req.method==='POST'){
    ...
    const r=createRun(body.moduleId,body.seed);db.prepare('INSERT INTO runs VALUES (?,?,?)').run(r.id,owner,JSON.stringify(r));send(res,201,publicRun(r));return;
   }
   ...
   if(!action&&req.method==='GET'){send(res,200,publicRun(r));return;}
   if(action==='export'&&req.method==='GET'){send(res,200,{schemaVersion:1,exportedAt:new Date().toISOString(),attempt:publicRun(r)});return;}
   if(action==='sandbox'&&req.method==='POST'){
    ...
    send(res,200,{simulated:true,status:body.timeout?504:old?200:201,body:body.timeout?{error:'Response timed out; capture outcome unknown. Retry with the same key.'}:{...value,replayed:!!old}});return;
   }
   ...
   r.updatedAt=new Date().toISOString();db.prepare('UPDATE runs SET body=? WHERE id=? AND owner=?').run(JSON.stringify(r),r.id,owner);send(res,200,publicRun(r));
